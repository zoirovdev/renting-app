...
const RandomProfilePage = () => {
    ...
    const copyPhone = async () => {
        try {
            if(randomUser){
                await navigator.clipboard.writeText(randomUser?.phone)
                setCopied(true)
        ...
        <div className='flex flex-col items-center gap-4 md:gap-8 py-4 md:py-8 px-4 md:px-0 pb-20 md:pb-8'>
            <div className="w-full max-w-[600px] py-4 px-4 md:px-6 space-y-4">
                <div className="flex flex-col justify-center items-center
                    ...
                    <p className="text-lime-600 dark:text-lime-500 text-lg md:text-xl">@{randomUser?.username}</p>
                    ...
                    <p className="dark:text-gray-100 text-xs md:text-sm text-center px-4">
                        ...
                        {new Date(randomUser?.created_at).toLocaleDateString('en-US', {
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric'
                        })}
